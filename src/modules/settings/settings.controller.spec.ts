...
describe("SettingsController", () => {
  ...
  describe("updateLanguage", () => {
    it("returns updated language for valid locale", async () => {
      mockSettingsService.updateLanguage.mockResolvedValue({
        language: "pt-BR",
      });

      const result = await controller.updateLanguage(mockRequest("u1"), {
        language: "pt-BR",
      });
    ...
    it("delegates correct userId and language to service", async () => {
      ...
      await controller.updateLanguage(mockRequest("user-xyz"), {
        language: "pt-BR",
      });
