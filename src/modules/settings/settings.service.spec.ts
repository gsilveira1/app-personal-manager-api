...
describe("SettingsService", () => {
  ...
  describe("getAiInstructions", () => {
    it("returns stored instructions when row exists", async () => {
      ...
      expect(result).toEqual({
        instructions: "Be concise and focus on hypertrophy.",
      });
    });
    ...
    it("calls findUnique with correct composite key", async () => {
      ...
      expect(mockPrismaService.userSetting.findUnique).toHaveBeenCalledWith({
        where: {
          userId_key: { userId: "user-456", key: "ai_prompt_instructions" },
        },
      });
    });
    ...
    it("bubbles up Prisma errors without masking", async () => {
      ...
      await expect(service.getAiInstructions("u1")).rejects.toThrow(
  ...
  describe("updateAiInstructions", () => {
    it("upserts with create payload on first write", async () => {
      ...
      const result = await service.updateAiInstructions(
        "u1",
    ...
    it("bubbles up Prisma errors without masking", async () => {
      ...
      await expect(service.updateAiInstructions("u1", "test")).rejects.toThrow(
  ...
  describe("getLanguage", () => {
    ...
    it("calls findUnique with correct composite key", async () => {
      ...
      expect(mockPrismaService.userSetting.findUnique).toHaveBeenCalledWith({
        where: {
          userId_key: { userId: "user-123", key: "preferred_language" },
        },
      });
    });
    ...
    it("bubbles up Prisma errors without masking", async () => {
      ...
      await expect(service.getLanguage("u1")).rejects.toThrow(
        "DB connection failed",
      );
    });
  });
