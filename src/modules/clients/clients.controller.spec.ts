...
describe("ClientsController", () => {
  ...
  it("PATCH /:id should delegate to update with userId, id, and body", async () => {
    ...
    expect(service.update).toHaveBeenCalledWith("trainer-uuid-1", "client-1", {
      name: "Updated",
    });
  });

  it("PATCH /:id/convert should delegate to convertLead with planId from body", async () => {
    service.convertLead!.mockResolvedValue({
      id: "client-1",
      status: "Active",
    } as any);
    ...
    expect(service.convertLead).toHaveBeenCalledWith(
      "trainer-uuid-1",
      "client-1",
      "plan-uuid",
    );
