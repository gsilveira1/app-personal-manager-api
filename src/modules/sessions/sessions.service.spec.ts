...
describe("SessionsService", () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: SettingsService, useValue: settingsService },
        {
          ...
          useValue: availabilityBlocksService,
        },
      ],
    }).compile();
  ...
  describe("create - conflict validation", () => {
    ...
    it("should throw ConflictException when time overlaps with availability block", async () => {
      ...
      await expect(service.create(userId, dto as any)).rejects.toThrow(
        "blocked by an availability block",
      );
    });
  ...
  describe("findOne", () => {
    ...
    it("should throw ForbiddenException when session belongs to another user", async () => {
      ...
      await expect(service.findOne(userId, sessionId)).rejects.toThrow(
        ForbiddenException,
      );
    });
  });
  ...
  describe("updateWithScope", () => {
    it("should update single session when scope is single", async () => {
      ...
      const result = await service.updateWithScope(
        userId,
        sessionId,
    ...
    it("should update all future sessions when scope is future and has recurrenceId", async () => {
      ...
      prisma.session.update.mockImplementation((args: any) =>
        Promise.resolve({ id: args.where.id, notes: "Updated" }),
      );
    ...
    it("should shift dates by timeDiff when data.date is provided with scope future", async () => {
      ...
      const futureSession1 = {
        ...mockSession,
        id: "session-2",
        recurrenceId,
        date: new Date("2025-02-08T10:00:00Z"),
      };
      const futureSession2 = {
        ...mockSession,
        id: "session-3",
        recurrenceId,
      ...
      await service.updateWithScope(
        userId,
        sessionId,
        { date: "2025-02-01T12:00:00Z" } as any,
        "future",
      );
      ...
      expect(updateCalls[0][0].data.date).toEqual(
        new Date("2025-02-01T12:00:00Z"),
      );
      expect(updateCalls[1][0].data.date).toEqual(
        new Date("2025-02-08T12:00:00Z"),
      );
      expect(updateCalls[2][0].data.date).toEqual(
  ...
  describe("createRecurringEvent", () => {
    ...
    it("should throw ForbiddenException when client belongs to another user", async () => {
      ...
      await expect(
        service.createRecurringEvent(userId, dto as any),
      ).rejects.toThrow(ForbiddenException);
    });
  ...
  describe("materializeRecurringSessionsForRange", () => {
    ...
    it("should apply rescheduled exceptions (non-cancelled with newStartTime)", async () => {
      ...
      expect(new Date(rescheduled!.date).toISOString()).toBe(
        "2025-01-07T14:00:00.000Z",
      );
    ...
    it("should combine occurrences from multiple recurring events", async () => {
      ...
      expect(
        result.filter((s) => s.recurringEventId === "event-1"),
      ).toHaveLength(2);
      expect(
        result.filter((s) => s.recurringEventId === "event-2"),
      ).toHaveLength(2);
    });
  ...
  describe("upsertSessionException", () => {
    ...
    it("should throw NotFoundException when recurring event does not exist", async () => {
      ...
      await expect(
        service.upsertSessionException(userId, {
          recurringEventId: "non-existent",
          originalStartTime: "2025-01-13T10:00:00Z",
        } as any),
  ...
  describe("removeRecurringEvent", () => {
    it("should delete entire recurring series", async () => {
      ...
      expect(prisma.recurringEvent.delete).toHaveBeenCalledWith({
        where: { id: "event-1" },
      });
    });
    ...
    it("should throw NotFoundException when event does not exist", async () => {
      ...
      await expect(
        service.removeRecurringEvent(userId, "non-existent"),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it("should throw ForbiddenException when event belongs to another user", async () => {
      ...
      await expect(
        service.removeRecurringEvent(userId, "event-1"),
      ).rejects.toThrow(ForbiddenException);
    });
  });
