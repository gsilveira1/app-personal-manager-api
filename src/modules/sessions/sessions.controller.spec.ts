...
describe("SessionsController", () => {
  ...
  describe("POST /recurring-event", () => {
    it("should delegate to service.createRecurringEvent", async () => {
      ...
      expect(service.createRecurringEvent).toHaveBeenCalledWith(
        "trainer-uuid-1",
        dto,
      );
    });
  ...
  describe("POST /:id/toggle-complete", () => {
    it("should delegate to service.toggleComplete", async () => {
      service.toggleComplete!.mockResolvedValue({
        id: "sess-1",
        completed: true,
      } as any);
      ...
      expect(service.toggleComplete).toHaveBeenCalledWith(
        "trainer-uuid-1",
        "sess-1",
      );
    });
  ...
  describe("PATCH /exception", () => {
    it("should delegate to service.upsertSessionException", async () => {
      ...
      expect(service.upsertSessionException).toHaveBeenCalledWith(
        "trainer-uuid-1",
        dto,
      );
    });
  ...
  describe("DELETE /recurring-event/:id", () => {
    it("should delegate to service.removeRecurringEvent", async () => {
      ...
      expect(service.removeRecurringEvent).toHaveBeenCalledWith(
        "trainer-uuid-1",
        "event-1",
      );
    });
  ...
  describe("GET /available (getAvailableSlots)", () => {
    ...
    it("should delegate to findAvailableSlots when TRAINER_USER_ID is set", async () => {
      ...
      const {
        SessionsController: FreshController,
      } = require("./sessions.controller");
      const { SessionsService: FreshService } = require("./sessions.service");

      const freshService = {
        findAvailableSlots: jest
          .fn()
          .mockResolvedValue([{ date: "2025-03-03", time: "10:00" }]),
      };
      ...
      const result = await freshController.getAvailableSlots(
        "2025-03-01",
        "2025-03-31",
      );
